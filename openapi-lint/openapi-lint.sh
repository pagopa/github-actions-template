#!/usr/bin/env bash
# Lints an OpenAPI spec with Spectral. Inputs arrive as env vars, see action.yml.
set -euo pipefail
...
# Spectral's severity numbering, as it appears in the JSON report.
...
# Exit 2 or more means Spectral could not run and wrote no JSON. Check the exit code and a
# non-empty report, or the gate passes silently and stops checking.
set +e
...
# unrecognized-format is only a warning, so fail_severity: error would let it through.
...
# Report-only blocks nothing, so the summary must not claim otherwise.
blocking() {
...
# Not delegated to Spectral's --fail-severity: report-only would then have to swallow its
# exit code, reopening the exit 2 hole above.
...
# info findings would bury them. They go to the job summary as counts instead.
...
# Downgrades the gate message only: finding annotations keep their severity.
GATE_LEVEL="error"
if [ "$FAIL_ON_VIOLATION" != "true" ]; then
  GATE_LEVEL="warning"
fi
